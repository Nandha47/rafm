...
# Ensure new config variables have defaults
if 'MANDATORY_ANCHOR_AGENTS' not in globals():
    MANDATORY_ANCHOR_AGENTS = ["VWAP", "OrderBook", "OrderFlow"]
if 'TRADING_HOURS_UTC' not in globals():
    TRADING_HOURS_UTC = [(6, 23)]
if 'MACRO_BEAR_THRESHOLD' not in globals():
    MACRO_BEAR_THRESHOLD = 0.98
if 'MACRO_BULL_THRESHOLD' not in globals():
    MACRO_BULL_THRESHOLD = 1.02
...
# ════════════════════════════════════════════════════════════════════════════
#  ML AGENTS — Train on actual trade PnL
# ════════════════════════════════════════════════════════════════════════════
class BaseMLAgent:
    FEATURE_COUNT = 11
                    ...
                    if qty_r > 0:
                        ...
                        if record and self.ml_agents_enabled and sym in self.ml_agents:
                            ...
                            if entry_idx is not None:
                                entry_history = history[:entry_idx+1]
                                entry_window = entry_history[max(0, entry_idx - 200):entry_idx+1]
                                if len(entry_window) >= 60:
                                    entry_ind = precompute_indicators(entry_window)
